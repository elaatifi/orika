...
public class Issue68TestCase {
    ...
    public void testSimpleReverceCase() {
        ...
        // InvoiceItem output = mapper.map(invoiceitem, InvoiceItem.class);
        
        InvoiceItemProxy invoiceItemProxy = BeanFactory.createInvoiceItemProxy();
        mapper.map(invoiceitemVO, invoiceItemProxy);
        
        // System.out.println(projectVO.name);
        // System.out.println(projectProxy.getProjectItems().size());
        
        // To be short
        // Assert.assertEquals(project.name, output.name);
        // Assert.assertEquals(projectVO.getProjectItems().size(),
        // projectProxy.getProjectItems().size());
