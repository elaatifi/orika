...
public class JavassistCompilerStrategy extends CompilerStrategy {
     ...
     * @throws IOException
     */
    protected void writeFiles(GeneratedSourceCode sourceCode,
	    CtClass byteCodeClass) throws IOException {

	if (writeClassFiles || writeSourceFiles) {
	    
	    String path = getClass().getResource("/").getFile().toString()
		    + sourceCode.getPackageName().replaceAll("\\.", "/") + "/";
	    ...
	    if (writeSourceFiles) {
		File sourceFile = new File(parentDir,
			sourceCode.getClassSimpleName() + ".java");
	...
	try {
		...
		try {
		    byteCodeClass.addField(CtField
			    .make(fieldDef, byteCodeClass));
		} catch (CannotCompileException e) {
		    LOG.error("An exception occured while compiling: "
			    + fieldDef + " for " + sourceCode.getClassName(), e);
		    throw e;
		}
	    }
		...
		try {
		    byteCodeClass.addMethod(CtNewMethod.make(methodDef,
			    byteCodeClass));
		} catch (CannotCompileException e) {
		    LOG.error("An exception occured while compiling: "
			    + methodDef + " for " + sourceCode.getClassName(),
			    e);
		    throw e;
		}
	    ...
	    throw new SourceCodeGenerationException(
		    "Could not write files for " + sourceCode.getClassName(), e);
	}
